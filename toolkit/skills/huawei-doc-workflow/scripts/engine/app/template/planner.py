...
def _match_replacements(required, available) -> list[TemplateReplacement]:
    ...
    for role, source_path, _text, _essential in required:
        compatible = [shape for shape in remaining if shape.role in aliases.get(role, (role,))]
        match = max(compatible, key=lambda shape: _source_slot_score(source_path, shape), default=None)
        if match is None:
            continue
...
def _source_slot_score(source_path: str, shape) -> int:
    preview = (shape.text_preview or "").casefold().replace(" ", "")
    keyword_groups = {
        "presenter": ("演讲人", "汇报人", "报告人", "presenter", "speaker"),
        "date": ("日期", "时间", "date", "time", "202", "20xx"),
        "index": ("序号", "章节", "section", "index"),
    }
    keywords = keyword_groups.get(source_path, ())
    return 10 if any(keyword in preview for keyword in keywords) else 0
