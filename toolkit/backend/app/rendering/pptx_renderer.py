...
def _kpi_card_content(shape, card, index: int, layout: dict, theme: dict) -> None:
    ...
    value_text = card.desc.replace(" ", "\u00a0")
    value_font_size = _fit_kpi_value_font_size(value_text, shape, layout, theme)
    for part_index, (text, size, bold, color_key, font_role) in enumerate(
        [
            (value_text, value_font_size, True, "hw_red", "number"),
...
def _fit_kpi_value_font_size(text: str, shape, layout: dict, theme: dict) -> float:
    maximum = float(layout["kpi_value_font_size_pt"])
    minimum = float(layout.get("kpi_value_min_font_size_pt", max(maximum - 8, 12)))
    ...
    # PowerPoint's Arial metrics are wider than the generic text-unit
    # estimator for strings such as "1.18 Gbps". Reserve horizontal safety
    # so fit-to-shape does not split a unit token inside the word.
    return _fit_ppt_text(
        text,
        candidates,
        {
            "width_in": (shape.width / 914400.0) * 0.84,
            "height_in": 0.7,
        },
        theme,
    )
...
def _architecture_node_boxes(
    ...
    for layer_index, (group_id, label, node_ids, _bounds) in enumerate(layers):
        ...
        for node_index, node_id in enumerate(node_ids):
            ...
            if group_id is None and _architecture_node_size(node, slide_ir) is None:
                # Graphviz-free runs must remain presentation-grade. The old
                # fixed 1.5 x 0.8 inch boxes left sparse four-node diagrams and
                # forced technical labels into clipped multi-line text. Scale
                # only automatic, ungrouped fallback nodes; explicit/manual
                # geometry and group-bound nodes retain their contract.
                width = min(
                    max(width, layout["node_width_in"] * 1.35),
                    max_node_width,
                )
                height = min(
                    max(height, layout["node_height_in"] * 1.4),
                    max_node_height,
                )
