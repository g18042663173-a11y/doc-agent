...
from doc_agent.ir.schemas import DeckIR, DocumentIR, to_pretty_json
...
class DeckPlanner:
    ...
    def plan(self, document_ir: DocumentIR, target_slide_count: int = 8) -> DeckIR:
        prompt = render_template(
            self._load_prompt("make_deck_ir.md"),
            {
    ...
    def _validate_or_repair(self, raw: dict[str, Any]) -> DeckIR:
        try:
            ...
            for _ in range(self.settings.max_repair_attempts):
                ...
                try:
                    ...
                    last_error = exc
                    bad_json = to_pretty_json(raw)
            raise ValueError(f"DeckIR validation failed after repair attempts: {last_error}") from last_error
