...
def _kpi_items(slide, slide_index: int, theme: dict) -> list[PptxLintItem]:
    ...
    for shape in shapes:
        ...
        maximum_size = float(layout["kpi_value_font_size_pt"])
        minimum_size = float(layout.get("kpi_value_min_font_size_pt", maximum_size))
        actual_size = value_run.font.size.pt if value_run.font.size is not None else 0
        if (
            value_run.font.name != expected_font
            or actual_size < minimum_size - 0.1
            or actual_size > maximum_size + 0.1
        ):
            return [
                _item(
                    "HW-W01",
                    "Warning",
                    slide_index,
                    f"KPI 核心数值未使用主题 number 字体或 {minimum_size:g}-{maximum_size:g}pt 自适应字号区间。",
                    "从 hw_theme.json 的 fonts.number、cards.kpi_value_font_size_pt 和 kpi_value_min_font_size_pt 读取样式。",
                )
            ]
...
def _font_size_variety_items(slide, slide_index: int, theme: dict, expected_classification: str) -> list[PptxLintItem]:
    ...
    for shape, run, _context in _iter_slide_runs(slide):
        if _is_footer_shape(shape, theme, expected_classification) or _uses_compact_typography(shape):
            continue
        if _is_kpi_value_run(shape, run):
            continue
...
def _is_kpi_value_run(shape, run) -> bool:
    shape_name = str(getattr(shape, "name", ""))
    if not (shape_name.startswith("KPI:") or ":KPI_CARD:" in shape_name) or not getattr(shape, "has_text_frame", False):
        return False
    paragraphs = shape.text_frame.paragraphs
    if not paragraphs or not paragraphs[0].runs:
        return False
    return run._r is paragraphs[0].runs[0]._r
