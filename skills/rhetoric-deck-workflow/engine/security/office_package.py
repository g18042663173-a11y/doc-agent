...
def preflight_office_package(path: Path, *, purpose: OfficePurpose = "source") -> OfficePackageReport:
    ...
    try:
        with ZipFile(path) as package:
            ...
            chart_workbooks = _chart_workbook_parts(internal_relationships, _chart_parts(package))
    ...
    for name in sorted(names):
        ...
        for relationship in root.findall(f"{{{REL_NS}}}Relationship"):
            ...
            if relationship.attrib.get("TargetMode") == "External":
                external.append(_external_relationship_label(name, relationship))
                continue
...
def _external_relationship_label(name: str, relationship: ElementTree.Element) -> str:
    relationship_type = relationship.attrib.get("Type", "").rsplit("/", 1)[-1].casefold()
    kind = "hyperlink" if relationship_type == "hyperlink" else "external"
    return f"{name}:{relationship.attrib.get('Id', '?')} ({kind})"


def _chart_parts(package: ZipFile) -> set[str]:
    """OPC part identities are defined by content types, not directory names."""
    root = ElementTree.fromstring(package.read("[Content_Types].xml"))
    charts = set()
    for entry in root:
        if entry.attrib.get("ContentType") == "application/vnd.openxmlformats-officedocument.drawingml.chart+xml":
            name = entry.attrib.get("PartName", "").lstrip("/")
            if name in package.namelist():
                chart = ElementTree.fromstring(package.read(name))
                if chart.tag == "{http://schemas.openxmlformats.org/drawingml/2006/chart}chartSpace":
                    charts.add(name)
    return charts


def _chart_workbook_parts(internal_relationships: list[tuple[str, str, str]], chart_parts: set[str] | None = None) -> set[str]:
    allowed: set[str] = set()
    for source, target, relationship_type in internal_relationships:
        if (
            (source in chart_parts if chart_parts is not None else re.fullmatch(r"ppt/charts/chart\d+\.xml", source, flags=re.IGNORECASE))
            and relationship_type == PACKAGE_REL_TYPE
            and target.casefold().endswith(".xlsx")
        ):
            allowed.add(target)
    return allowed
